	...
	public function send() {
		...
		foreach ($this->attachments as $attachment) {
			if (file_exists($attachment)) {
				...
				$message .= 'Content-ID: <' . basename($attachment) . '>' . $this->newline; //basename(urlencode($attachment))
				$message .= 'X-Attachment-Id: ' . basename($attachment) . $this->newline . $this->newline; //basename(urlencode($attachment))
		...
		if ($this->protocol == 'mail') {
			...
			if ($this->parameter) {
				if(!mail($to, '=?UTF-8?B?' . base64_encode($this->subject) . '?=', $message, $header, $this->parameter)){
				 throw new Exception('Error: non è stato possibile inviare l\'email');	
				}
			} else {
				if(!mail($to, '=?UTF-8?B?' . base64_encode($this->subject) . '?=', $message, $header)){
					throw new Exception('Error: non è stato possibile inviare l\'email');	
				}
			}
			...
			if (substr($host, 0, 6) === 'ssl://') {
				$scheme = 'ssl://';
				$host = substr($host, 6);
			} elseif (substr($host, 0, 6) === 'tls://') {
				$scheme = 'tls://';
				$host = substr($host, 6);
			}

			$handle = fsockopen($scheme . $host, $this->port, $errno, $errstr, $this->timeout);
			...
			throw new Exception('Error: ' . $errstr . ' (' . $errno . ')');			
				...
				if (!$scheme && $this->starttls) {
					...
					throw new Exception('Error: STARTTLS negotiation failed!');	
